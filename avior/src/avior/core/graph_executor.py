...
class GraphExecutor:
    ...
    def _execute_node(
        ...
        #CHANGED
        # Get the operator's signature
        signature = node.operator.get_signature()
        if signature:
            # Pass through all input fields directly
            return node.operator(inputs=input_data)
        
        # Regular return for operators without signatures
        return node.operator(inputs=input_data)
