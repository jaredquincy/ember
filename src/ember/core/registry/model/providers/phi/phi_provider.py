...
class PhiProvider(BaseProviderModel):
    """Provider for Microsoft's Phi models using HuggingFace Transformers.
    
    This provider supports running Phi models locally using the transformers
    library. It handles efficient inference with optional quantization.
    """
    
    def __init__(self, model_info: ModelInfo) -> None:
        """Initialize the Phi provider.
    ...
    def create_client(self) -> Dict[str, Any]:
        """Create Phi model and tokenizer instances.
        ...
        model = AutoModelForCausalLM.from_pretrained(
            ...
            torch_dtype=torch.float16  # Phi works best with float16
        )
        tokenizer = AutoTokenizer.from_pretrained(
            self.model_path,
            trust_remote_code=True,
            padding_side="left"  # Phi specific setting
        )
        
        # Phi specific tokenizer setup
    ...
    def forward(self, request: ChatRequest) -> ChatResponse:
        ...
        inputs = self.client["tokenizer"](
            ...
            padding=True,
            truncation=True,
            max_length=self.model_info.context_length or 2048
        ).to(self.device)
        ...
        with torch.no_grad():
            outputs = self.client["model"].generate(
                **inputs,
                ...
                repetition_penalty=1.1  # Phi specific setting
            )
    ...
    def _format_prompt(self, prompt: str) -> str:
        """Format the prompt for Phi chat models.
